...
var sql = "SELECT ID,PRJ_ID,PRJNO,PRJ_NAME,FUN_GETCODEDESC('DEPT_ID',DEPT_ID) DEPT_ID,PROGRESS,TO_CHAR(CHECKDATE,'YYYY-MM-DD') CHECKDATE,CHECK_USER,(SELECT EMP_NAME FROM HRD_EMP WHERE EMP_ID = V_PRJ_MAJORCHECK.TESTER) TESTER,MEMO,VALID,(SELECT EMP_NAME FROM HRD_EMP WHERE EMP_ID = V_PRJ_MAJORCHECK.EMP_ID) EMP_ID,(SELECT EMP_NAME FROM HRD_EMP WHERE EMP_ID = V_PRJ_MAJORCHECK.EMP_ID_2) EMP_ID_2,SUM1,RATIO1*100 RATIO1,CHECKGROUP_NO  FROM V_PRJ_MAJORCHECK WHERE CHECK_TYPE='02'";
// 定义数据类型
var dsOption = {
	fields : [ {
		name : "ID"
...
var gridOption = {
	id : "projectcheck_building_grid",
	loadURL : MyURL + 'common/doPageQuery?sql=' + sql,
	width : "100%", // "100%", // 700,
	height : c - header - 67, // "100%", // 330,
	container : "gridbox_projectcheck_building",
...
var projectcheck_building_grid = new Sigma.Grid(gridOption);
projectcheck_building_grid.render();
