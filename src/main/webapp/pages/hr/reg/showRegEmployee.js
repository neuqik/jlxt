...
var header = $("#showregemployee_head").height();
...
var regwhere = $("#regwhere").val();
if (regwhere.length > 0) {
	sql = sql + regwhere;
}
...
var gridOption = {
	...
	onCellDblClick : function(value, record, cell, row, colNO, rowNO,
			columnObj, grid) {
		navTab.openTab("zcbj", MyURL + "labour/showEmpReg?empId="
				+ record.EMP_ID, {
			title : record.EMP_ID + " 注册",
			fresh : false,
			data : {}
		});
	}
...
var showregemployee_grid = new Sigma.Grid(gridOption);
showregemployee_grid.render();
