...
var header = $("#showlabouremployee_head").height();
...
var labourwhere = $("#labourwhere").val();
if (labourwhere.length > 0) {
	sql = sql + labourwhere;
}
...
var gridOption = {
	...
	onCellDblClick : function(value, record, cell, row, colNO, rowNO,
			columnObj, grid) {
		navTab.openTab("ldgx", MyURL + "labour/editEmpLabour?empId="
				+ record.EMP_ID, {
			title : "劳动关系",
			fresh : false,
			data : {}
...
var showlabouremployee_grid = new Sigma.Grid(gridOption);
showlabouremployee_grid.render();
