...
var navTab = {
		...
		$obj.contextMenu('navTabCM', {
			bindings : {
				...
				closeOther : function(t, m) {
					...
					$this._closeOtherTab(index > 0 ? index
							: $this._currentIndex);
				},
			...
			ctrSub : function(t, m) {
				...
				if ($this._currentIndex == 0
						|| t.attr("tabid") == $this._op.mainTabId) {
					mCur.addClass("disabled");
	...
	_scrollNext : function() {
		...
		if (iEnd < this._getTabs().size()) {
			this._scrollTab(-this._getTabsW(0, iEnd + 1)
					+ this._getScrollBarW());
		}
	},
	...
	_scrollCurrent : function() { // auto scroll current tab
		...
		if (iW <= this._getScrollBarW()) {
			...
			this._scrollTab(this._getScrollBarW()
					- this._getTabs().eq(this._currentIndex).outerWidth(true)
	...
	_switchTab : function(iTabIndex) {
		var $tab = this._getTabs().removeClass("selected").eq(iTabIndex)
				.addClass("selected");
		this._getPanels().hide().eq(iTabIndex).show();

		this._getMoreLi().removeClass("selected").eq(iTabIndex).addClass(
				"selected");
	...
	closeCurrentTab : function() {
		if (this._currentIndex > 0) {
			this._closeTab(this._currentIndex);
		}
	},
	...
	reload : function(url, options) {
		...
		var $panel = op.navTabId ? this._getPanel(op.navTabId) : this
				._getPanels().eq(this._currentIndex);
		if ($panel) {
			if (!url) {
				var $tab = op.navTabId ? this._getTab(op.navTabId) : this
						._getTabs().eq(this._currentIndex);
				url = $tab.attr("url");
			}
			if (url) {
	...
	openExternal : function(url, $panel) {
		...
		$panel.html(DWZ.frag["externalFrag"].replaceAll("{url}", url)
	...
	openTab : function(tabid, url, options) { // if found tabid replace tab,
		...
		// wonder add for open 8
		if (this._getTabs().length >= 8) {
			alertMsg.error("打开窗口数量太多，请关闭一些重试！");
			return;
		}
		...
		if (iOpenIndex >= 0) {
			...
			var stSpan = $tab.attr("tabid") == this._op.mainTabId ? "> a > span > span"
					: "> a > span";
			...
			this._tabBox.append(tabFrag.replace("#tabid#", tabid).replaceAll(
					"#title#", op.title));
			this._panelBox.append('<div class="page"></div>');
			this._moreBox
					.append('<li><a href="javascript:" title="#title#">#title#</a></li>'
